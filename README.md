...
本项目构建了一套完整的A股量化投研系统，涵盖三大模块：（1）基于财务因子（PE/PB/ROE/增速等）的股票风格分类与随机森林归因分析，结合LLM对研报公告进行语义解读，生成可解释的涨跌归因；（2）季频财务数据驱动的选股验证框架；（3）基于LightGBM + 77个自研因子的日频量化交易系统，采用滚动训练机制（训练数据截止信号日前7天以规避未来信息），T-1信号T日执行的严格回测框架，在2025年5-12月测试期实现夏普比率2.74、最大回撤15.90%、超额收益55.46%（vs沪深300），并集成QMT的自动化交易链路，支持实盘部署。
